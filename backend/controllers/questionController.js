...
const createQuestion = async (req, res) => {
  try {
    ...
    const question = await Question.create({ title, description, tags: cleanTags, userId: req.user._id });
...
// @desc    Mark / unmark answer as solution
...
// @desc    Get current user's own questions
...
const getMyQuestions = async (req, res) => {
  try {
    const questions = await Question.aggregate([
      ...
      { $project: { title: 1, description: 1, tags: 1, isSolved: 1, answerCount: 1, createdAt: 1 } },
      { $sort: { createdAt: -1 } }
    ]);
